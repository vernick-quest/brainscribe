...
function OnboardingIcon({ userId, complete, practiced }) {
  ...
  const { Glyph, color, title } = {
    ...
    skipped: { Glyph: IconSkipped, color: 'var(--status-warning, #D97706)', title: 'Skipped — marked onboarded WITHOUT finishing a practice assignment. Click to reset' },
...
function StudentRosterLegend() {
  ...
  return (
    <div className="flex items-center gap-5 flex-wrap px-5 pt-3 text-xs"
      ...
      {item(IconSkipped, 'Skipped', 'var(--status-warning, #D97706)')}
    ...
    <div className="rounded-2xl overflow-hidden"
      ...
      <div className="px-5 py-3 flex items-center gap-4">
        ...
        <span className={`${COL} text-xs font-semibold`}
          ...
          style={{ color: !warn?.total ? 'var(--text-subtle)' : warn.high > 0 ? 'var(--status-error, #DC2626)' : 'var(--status-warning, #D97706)' }}>
          {warn?.total || '—'}
        </span>
