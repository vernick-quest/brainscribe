...
// Typed portfolio renders — the pair/blueprint shapes ARE the growth evidence, so they
// render structurally, not as flattened text (design §The Writing Portfolio). Shared by
// the student's own portfolio (/skill-studio/portfolio) and a parent viewing a child's
// portfolio (/skill-studio/portfolio?student=<id>). Server component — no client state.
...
export default function GymPortfolioView({ entries = [], heading, subheading, emptyText, selfAssessmentPrefix = 'In their words:' }) {
  return (
    <>
      <h1 style={{ font: 'var(--type-title)', color: 'var(--text-strong)', margin: '8px 0 4px' }}>{heading}</h1>
      {subheading && (
        <p style={{ font: 'var(--type-body)', color: 'var(--text-muted)', marginBottom: 24 }}>{subheading}</p>
      )}
      ...
      {entries.length === 0 ? (
        <div className="p-8 text-center" style={{ background: 'var(--surface-card)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-default)' }}>
          <p style={{ font: 'var(--type-body)', color: 'var(--text-muted)', margin: 0 }}>{emptyText}</p>
        </div>
      ) : (
              ...
              <article key={entry.id} className="p-5" style={{ background: 'var(--surface-card)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border-default)' }}>
                ...
                {entry.self_assessment && (
                  <p style={{ font: 'var(--type-meta)', color: 'var(--text-muted)', margin: '12px 0 0', fontStyle: 'italic' }}>
                    {selfAssessmentPrefix} {entry.self_assessment}
                  </p>
                )}
              </article>
            )
