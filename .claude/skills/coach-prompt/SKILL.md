...
- For behavior tuning also: `brainscribe-personas-reference.md` (CURRENT persona methods + test sheet), `brainscribe-scaffold-improvements.md`, `brainscribe-strict-reflection.md`. (`brainscribe-personas-complete.md` is the superseded Marcus/Verity-era framework — rationale only.)

## Invariants to preserve
- **Token contract** — `[SCAFFOLD] [ACTIVE] [NUGGET] [DONE] [THESIS] [PARA_DONE] [DICTATE] [COMPLETE]` (`[DICTATE]` gates the mic/dictation phase in `TutorSession.js` — dropping it silently kills the dictation handoff). The server strips them via `TOKEN_RE` in `app/api/tutor/route.js`; consumers parse them in `components/TutorSession.js`, `app/api/sessions/[id]/complete/route.js`, and `app/api/scaffold/[sessionId]/route.js`. Renaming or reformatting a token silently breaks the document panel, progress tracking, and session completion — **update every consumer in the same change** (grep the token names first).
