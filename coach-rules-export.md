# BrainScribe — Coach Rules Export

_Exact rule text injected into every coach system prompt. A persona's full system
prompt = its persona block (below) with the two SHARED blocks — Core Guardrails and
Structural Coaching Rules — spliced in where its `[INJECT …]` markers sit._

Generated from `lib/prompts.js` for an external differential review.

---

# SHARED — Core Guardrails
_(identical across all six personas)_
...
# SHARED — Structural Coaching Rules
_(identical across all six personas)_
...
# PER-PERSONA BLOCKS
_(persona-specific voice; the two `[INJECT …]` markers show where the shared blocks above are spliced in)_
