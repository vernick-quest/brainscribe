...
--
-- 051 declared this as an INLINE column check, so its name was auto-generated by
-- Postgres. `drop constraint if exists <guessed name>` would SILENTLY NO-OP if the
-- live name differs — leaving the old constraint in place beside the new one, so
-- every 'lock' insert fails forever against a constraint nobody remembers dropping.
-- "if exists" makes that failure invisible, which is the same trap as running
-- `create table if not exists` against a table that is already there. So: find the
-- real name and drop whatever is actually enforcing it.
do $$
declare c record;
begin
  for c in
    select conname from pg_constraint
    where conrelid = 'public.provenance_checks'::regclass
      and contype = 'c'
      and pg_get_constraintdef(oid) ilike '%trigger%'
  loop
    execute format('alter table public.provenance_checks drop constraint %I', c.conname);
    raise notice 'dropped trigger check constraint: %', c.conname;
  end loop;
end $$;
...
-- Re-runnable: adding a constraint that already exists is an error, not a no-op.
alter table public.provenance_checks
  drop constraint if exists provenance_checks_kind_check;
alter table public.provenance_checks
