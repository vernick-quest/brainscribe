...
export const WAITLIST_STATES = {
  ...
  subscriber:{ label: 'Blog',       needsAction: false, hint: 'Signed up for new posts, not for access.' },
...
// The landing-page form and the blog form write to the SAME table, distinguished only
// by `source`. Only an access request belongs in an approval queue: mailing an access
// code to someone who asked for new posts is answering a question they never asked.
export const ACCESS_REQUEST_SOURCES = new Set(['waitlist', 'landing'])

export function isAccessRequest(row = {}) {
  return ACCESS_REQUEST_SOURCES.has(String(row.source ?? '').toLowerCase())
}
...
export function classifySubscriber(row = {}, account = null) {
  ...
  // A blog subscriber is not waiting for anything. Without this they render as
  // "Waiting" with an Approve & send button, and the first blog signup becomes a
  // person we mail an access code to for no reason.
  if (!isAccessRequest(row)) return 'subscriber'
  return 'waiting'
}
...
export function buildWaitlistView(rows = [], accountsByEmail = {}, now = new Date().toISOString()) {
  ...
  const counts = { waiting: 0, invited: 0, signed_up: 0, writing: 0, dismissed: 0, subscriber: 0, stalled: 0 }
