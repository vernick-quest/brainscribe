...
import { classifySubscriber, daysWaiting, buildWaitlistView, isAccessRequest } from './waitlist.js'
...
const row = (over = {}) => ({ email: 'a@example.com', source: 'waitlist', created_at: '2026-08-01T12:00:00Z', ...over })
...
describe('source', () => {
  // The landing form and the blog form share this table. Without a source check the
  // first blog signup renders as "Waiting" with an Approve & send button.
  it('a blog subscriber is never an access request', () => {
    expect(classifySubscriber(row({ source: 'blog' }), null)).toBe('subscriber')
  })

  it('and never counts toward the action badge', () => {
    const { needsAction, counts } = buildWaitlistView(
      [row({ email: 'reader@example.com', source: 'blog' }), row({ email: 'asked@example.com', source: 'waitlist' })],
      {}, NOW,
    )
    expect(needsAction).toBe(1)
    expect(counts.subscriber).toBe(1)
  })

  it('but a blog subscriber who signs up is still shown as a real user', () => {
    expect(classifySubscriber(row({ source: 'blog' }), { sessionCount: 2, turnCount: 5 })).toBe('writing')
  })
})
...
describe('buildWaitlistView', () => {
  ...
  it('is empty and zeroed, not undefined, with no rows', () => {
    expect(buildWaitlistView([], {}, NOW)).toEqual({ items: [], counts: { waiting: 0, invited: 0, signed_up: 0, writing: 0, dismissed: 0, subscriber: 0, stalled: 0 }, needsAction: 0 })
  })
})
