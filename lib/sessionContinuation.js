...
// ── KILL SWITCH — off since 2026-08-08 ───────────────────────────────────────
// A v2 carries v1's `current_paragraph_index` verbatim, and a finished v1 parks it
// at the all-done sentinel `components.length`. TutorSession's saveParagraph then
// clamps the next cursor to `Math.min(sectionIndex + 1, components.length)` — which
// does not ADVANCE when it already starts at the sentinel. So a second dictated
// addition recomputes the SAME position, and /api/paragraphs upserts on
// (session_id, position): the second addition silently overwrites the first.
//
// That is the seventh silent drop path, and it lands on exactly the student who
// reopened a draft to add more. Derived by reading the path twice (the lane, then
// the conductor) but NOT yet reproduced end-to-end, which is why the feature is
// switched off rather than patched on a hunch — a fix built on an unreproduced
// theory is how the last three drop-path repairs introduced new ones.
//
// Existing v2 sessions keep working; this only stops NEW ones being minted.
// Flip back to true when a sentinel test reproduces the loss AND then passes:
// plant text, make two separate dictated additions, read both back.
export const CONTINUATION_ENABLED = false
