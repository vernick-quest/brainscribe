...
export function isPrivateIpv6(str) {
  ...
  if (!s) return true
  // ANY address that normalizes to a leading "::" has its high 96+ bits zero:
  // loopback (::1), unspecified (::), IPv4-mapped — in BOTH the dotted
  // ::ffff:127.0.0.1 AND the WHATWG-normalized hex ::ffff:7f00:1 forms — and the
  // deprecated IPv4-compatible (::a.b.c.d). None are legitimate public hosts, so
  // reject the whole class. (A dotted-tail regex missed the hex-normalized mapped
  // form → private-address SSRF; this closes it. Global-unicast v6 is 2000::/3, so
  // no legit public address starts with "::".)
  if (s.startsWith('::')) return true
  if (/^64:ff9b:/.test(s)) return true                       // NAT64 well-known prefix (embeds a v4)
  if (/^2002:/.test(s)) return true                          // 6to4 (embeds a v4) — reject the class
