...
export async function grantConsentForPending(service, { pending, parentUserId, ip = null, userAgent = null, method = 'email_plus' }) {
  ...
  // Access ONLY — NOT a Beta Circle slot (that comes from the code or grandfathering).
  await service
    .from('profiles')
    .update({
  ...
  // (access only, no Beta Circle) so a guardian who created an account purely to
  // approve their child isn't gated out of the product. Separate write; the parent
  // may already be access_granted (grandfathered / their own code), which is a no-op.
  await service
    .from('profiles')
