...
export async function grantConsentForPending(service, { pending, parentUserId, ip = null, userAgent = null, method = 'email_plus' }) {
  ...
  // Beta-launch access-gate inheritance (migration 045): a student who completes
  // parental consent is linked to a guardian and must be able to reach the coach, so
  // inherit ACCESS here (access_granted) exactly as an invited user does — otherwise a
  // COPPA-approved under-13 would clear the hardest flow only to hit the code gate.
  await service
    .from('profiles')
    .update({
  ...
  // The consenting parent is likewise linked to this student — grant them access too
  // so a guardian who created an account purely to approve their child isn't gated
  // out. Idempotent — they may already be access_granted (grandfathered / own code).
  await service
    .from('profiles')
    .update({ access_granted: true })
    .eq('id', parentUserId)

  // The approved STUDENT takes a Beta Circle slot up to the fluid cap of 100 (the
  // cohort counts students only — the parent gets access above but never a slot).
  // Best-effort — the cap + student-only checks live in maybeGrantBetaCircle; it
  // never blocks consent.
  await maybeGrantBetaCircle(service, pending.student_id)
...
// ── PARENT-LED consent (parent-first flow) ───────────────────────────────────────────
//
// The grant for the inverted flow: no pending record exists, because nothing was ever
// pending. A parent independently created their own account, proved their own email
// through OAuth, and affirmatively invited a child; the child then claimed that invite.
// Consent is a by-product of those acts rather than a link the child caused to be clicked.
//
// `pending_id` is nullable on coppa_consent_log (migration 006), so this needs no schema
// change — the audit row simply has no pending record to point at, which is the honest
// representation of what happened.
//
// The caller MUST have run evaluateParentLedConsent first; this function trusts that
// decision and does not re-derive it. Keeping the rules in one pure, tested place is the
// point — see lib/parentFirst.js for what they are and why.
//
// Idempotent: the profile update is guarded on coppa_consent_given still being false, so a
// re-claimed invite can't double-log or overwrite an earlier, truer timestamp.
export async function grantParentLedConsent(service, { childId, parentUserId, ip = null, userAgent = null }) {
  // Atomically claim: only the write that flips false → true proceeds to log.
  ...
  if (!claimed?.length) return { ok: true, alreadyDone: true }

  // The consenting parent is linked to this child and must not be gated out.
  await service.from('profiles').update({ access_granted: true }).eq('id', parentUserId)

  // Student-only Beta Circle slot, capped. Best-effort — never blocks consent.
  await maybeGrantBetaCircle(service, childId)

  await service.from('coppa_consent_log').insert({
    student_id: childId,
    parent_id: parentUserId,
    pending_id: null,
    consent_method: 'parent_led',
    ip_address: ip,
    user_agent: userAgent,
    privacy_policy_version: 'v1.0-june-2025',
  })
