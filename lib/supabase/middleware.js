...
export async function updateSession(request) {
  ...
  const publicPaths = ['/login', '/invite', '/api/auth', '/api/cron', '/api/admin/backfill-writing-profiles', '/api/admin/backfill-greetings', '/api/subscribe', '/api/unsubscribe', '/privacy', '/terms', '/about', '/blog', '/coppa/consent', '/coppa/confirm', '/welcome', '/faq', '/compare', '/writing-help']
  ...
  await recordPresence(request, supabaseResponse, user)

  return supabaseResponse
}

// ── Presence ────────────────────────────────────────────────────────────────────
// Stamp profiles.last_seen_at so the admin roster can answer "is this person using
// the app?". Nothing else can: auth.users.last_sign_in_at only moves on a FRESH
// sign-in (an iPad that stays logged in refreshes silently for weeks), and
// sessions.last_active_at only moves on an actual coach/student turn — so reading or
// browsing registered as nothing at all.
//
// Throttled by a short-lived cookie rather than by reading the profile first: if the
// cookie is present we've stamped within PRESENCE_TTL, so we skip. That's ONE write
// per user per 5 minutes and ZERO extra reads on every other request.
//
// Best-effort by construction — presence is a nice-to-have, so a failure here must
// never delay or break a page load.
const PRESENCE_TTL = 300 // seconds
const PRESENCE_COOKIE = 'bs_seen'

async function recordPresence(request, response, user) {
  try {
    if (!user?.id) return
    if (request.cookies.get(PRESENCE_COOKIE)) return // stamped recently
    // Skip asset-ish and cron traffic — presence should mean a person, not a poller.
    const { pathname } = request.nextUrl
    if (pathname.startsWith('/api/cron') || pathname.startsWith('/_next')) return

    // Set the throttle cookie FIRST so a slow/failed write can't cause a stamp storm.
    response.cookies.set(PRESENCE_COOKIE, '1', {
      maxAge: PRESENCE_TTL,
      httpOnly: true,
      sameSite: 'lax',
