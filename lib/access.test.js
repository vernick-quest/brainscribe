import { accessCodeAlertLevel } from '@/lib/access'
import { describe, it, expect } from 'vitest'

// Hitting a code's ceiling silently decommissions it; this decides when to say so.
// It must fire exactly once per threshold WITHOUT stored state — safe because a
// successful claim increments `uses` by exactly 1.
describe('accessCodeAlertLevel', () => {
  it('never alerts on an uncapped code', () => {
    for (const max of [null, undefined, 0, -1, '100']) {
      expect(accessCodeAlertLevel(50, max)).toBe(null)
    }
  })

  it('alerts once at 80% and once at the cap, and never in between', () => {
    const fired = []
    for (let uses = 1; uses <= 100; uses++) {
      const level = accessCodeAlertLevel(uses, 100)
      if (level) fired.push([uses, level])
    }
    expect(fired).toEqual([[80, 'warning'], [100, 'cap']])   // exactly two, no repeats
  })

  it('goes quiet once the cap is passed (already reported)', () => {
    expect(accessCodeAlertLevel(101, 100)).toBe(null)
    expect(accessCodeAlertLevel(999, 100)).toBe(null)
  })

  it('rounds the warning point up so it lands before the ceiling', () => {
    expect(accessCodeAlertLevel(4, 5)).toBe('warning')   // ceil(5 * 0.8) = 4
    expect(accessCodeAlertLevel(5, 5)).toBe('cap')
  })

  it('lets the cap speak alone when a tiny limit collides with the warn point', () => {
    // max 1 → ceil(0.8) = 1 = the ceiling. One event, not two.
    expect(accessCodeAlertLevel(1, 1)).toBe('cap')
    expect(accessCodeAlertLevel(2, 2)).toBe('cap')
    expect(accessCodeAlertLevel(1, 2)).toBe(null)
  })

  it('ignores non-integer input rather than guessing', () => {
    expect(accessCodeAlertLevel('80', 100)).toBe(null)
    expect(accessCodeAlertLevel(80.5, 100)).toBe(null)
    expect(accessCodeAlertLevel(NaN, 100)).toBe(null)
  })
})
import {
