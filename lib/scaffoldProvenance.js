...
export function sessionCoachContribution(components = []) {
  ...
  for (const p of components ?? []) {
    ...
    for (const rec of records) {
      ...
      // ── REFERENCED LOCKS ARE EXCLUDED FROM THE RATIO (SPEC-lock-by-reference.md) ────
      // A lock resolved by reference took its text out of the student's own message, so
      // its novelFraction is 0 BY CONSTRUCTION — not because the coach stayed out of the
      // way. Left in, it contributes 0 to the numerator and its FULL LENGTH to the
      // denominator, so it drags coachContribRatio down weighted by passage length, and a
      // long referenced lock can mask real coach authorship elsewhere in the same session.
      //
      // Both failures point the reassuring way, which is how every loss in this repo has
      // presented. A threshold calibrated on a mixed echoed+referenced corpus is calibrated
      // on two different distributions averaged together; promotion has never fired (0/18),
      // so it is untested rather than proven safe, and this would be its first live outing.
      //
      // Counted and reported, never silently dropped: `referencedCount` is what makes a
      // mixed corpus VISIBLE to whoever calibrates.
      if (rec.referenced) { referencedCount++; continue }
  ...
  return {
    ...
    flaggedCount,
    referencedCount,
    // Locks the ratio is actually derived from. checkedCount - referencedCount, surfaced
    // so a caller never has to infer whether a 0.0 means "clean" or "nothing scored".
    scoredCount: checkedCount - referencedCount,
