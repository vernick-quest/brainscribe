...
export function signalFingerprint(flag) {
  ...
  const canonical = {
    ...
    // Both added 2026-08-17. They MUST be in the fingerprint: a verdict answered the
    // session as it was, and a NEW refused revision or a newly-incremented over-claim
    // counter is exactly the kind of change that must resurface a 'resolved' row.
    // Leaving them out would let a fresh, recorded loss hide behind an old shrug.
    refusedRevisions: normList(flag.refusedRevisions, 'id', 'label'),
    lockOverClaims: asNum(flag.lockOverClaims),
  }
