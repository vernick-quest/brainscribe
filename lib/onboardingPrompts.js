...
export const PROMPTS_SELF_KNOWLEDGE = [
  {
    ...
    icon: 'target',
    label: "Something you're good at",
  ...
  },
  {
    ...
    category: 'self_knowledge',
    icon: 'trending',
    label: "Something you got better at",
...
export const PROMPTS_OPINIONS = [
  {
    ...
    category: 'opinion',
    icon: 'film',
    label: "Something that stuck with you",
  ...
  },
  {
    ...
    category: 'opinion',
    icon: 'school',
    label: "One change to your school",
...
// Owen's opening line for the hook-only practice. Shared so the server can persist
// it as the first message when the practice session is created (keeping it in the
// transcript and on return), and the client greeting falls back to the same text.
export function onboardingGreeting(name = 'there', prompt = null) {
  // Use the chosen prompt's topic-specific opener so Owen names the theme back to
  // the student in the chat — it helps them remember what they picked.
  if (prompt?.coach_opener) {
    return `Okay ${name} — let's find your topic. ${prompt.coach_opener}`
  }
  return `Okay ${name} — first let's pick what you actually want to write about for this. Don't overthink it: what's the first thing that comes to mind?`
}
