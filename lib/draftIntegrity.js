...
export function checkDraftIntegrity(paragraphs = [], components = [], opts = {}) {
  ...
  if (isComplete && unfilled.length > 0 && droppedComponents.length === 0) {
    reasons.push(
    ...
    // exact loss this module was written for; the healthy lookalikes (Lyndsay's quick
    // write, the restored sessions) are the ones with no word target at all.
