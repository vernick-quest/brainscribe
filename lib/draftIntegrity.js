...
//
// ── Why membership isn't enough (red-team, 2026-08-04) ───────────────────────────────
// Asking "does this word appear ANYWHERE in the draft" says a sentence survived whenever
// the essay happens to reuse its vocabulary — which a conclusion does by construction,
// since restating the thesis is its job. A wholly-absent closing paragraph could score
// above the threshold and be reported as present.
//
// So survival is measured as an ORDERED subsequence (longest common subsequence) of the
// sentence's content words within the draft's. That threads the needle:
//   · a lightly reworded sentence still matches — a couple of word swaps leave the rest of
//     the sequence intact, which is the scribe-edit case this tolerance exists for;
//   · a deleted sentence whose vocabulary is scattered across OTHER sentences does not,
//     because those words do not appear in its order.
// A contiguous-run measure was tried first and was too strict: one swapped word in the
// middle ("a thing" -> "a tool") split the run and reported the whole sentence lost.
function orderedOverlapRatio(words, finalWords) {
  if (!words.length) return 0
  // Classic LCS, rolling row. Sentences are short and drafts are a few hundred words, so
  // this is cheap enough to run per sentence per session.
  let prev = new Array(finalWords.length + 1).fill(0)
  for (let i = 1; i <= words.length; i++) {
    const cur = new Array(finalWords.length + 1).fill(0)
    for (let j = 1; j <= finalWords.length; j++) {
      cur[j] = words[i - 1] === finalWords[j - 1]
        ? prev[j - 1] + 1
        : Math.max(prev[j], cur[j - 1])
    }
    prev = cur
  }
  return prev[finalWords.length] / words.length
}

function survivalRatio(text, finalNorm) {
  const words = norm(text).split(' ').filter(w => w.length > 3)
  // A sentence with no content words ("I am so mad at my mom.") used to return 1 —
  // unconditionally "present", so never counted as missing anywhere. Treat it as absent
  // and let the containing item's own orphan verdict decide.
  if (!words.length) return 0
  const finalWords = finalNorm.split(' ').filter(w => w.length > 3)
  return orderedOverlapRatio(words, finalWords)
}
...
export function checkDraftIntegrity(paragraphs = [], components = [], opts = {}) {
  const {
    ...
    brokenCommitments = [], lockOverClaims = 0,
  } = opts

  // Revisions the scaffold-write guard REFUSED. resolveComponentWrite stamps
  // `revisionRefused` on the item when it declines a cross-section overwrite or an
  // inexact match, and until now nothing read it — recorded but never surfaced is the
  // half-finished shape that lets a signal look healthy. lib/prompts.js mandates
  // [DONE:thesis:…] alongside [THESIS:…], so this path fires routinely: a refusal means
  // the coach tried to write somewhere the guard would not let it, and the student may
  // have been told that edit landed.
  const refusedRevisions = (Array.isArray(components) ? components : [])
    .flatMap(sec => (Array.isArray(sec?.items) ? sec.items : []))
    .filter(it => it?.revisionRefused)
    .map(it => ({ id: it.id ?? it.label ?? 'component', kind: it.revisionRefused }))
  ...
  // A refused cross-section write is the one that matters most: the guard stopped a
  // coach writing into a DIFFERENT section than the one it named, which means the edit
  // the student watched be announced did not land where they were told.
  if (refusedRevisions.length > 0) {
    const cross = refusedRevisions.filter(r => r.kind === 'cross-section')
    const inexact = refusedRevisions.filter(r => r.kind === 'inexact')
    if (cross.length > 0) {
      reasons.push(
        `${cross.length} revision(s) REFUSED as cross-section: ` +
        `${cross.map(r => r.id).join(', ')} — the coach tried to overwrite a different ` +
        `section than the one it named, so that edit never landed where the student was told`
      )
      severity = 'alert'
    }
    if (inexact.length > 0) {
      reasons.push(
        `${inexact.length} revision(s) refused on an inexact match: ` +
        `${inexact.map(r => r.id).join(', ')} — the guard preserved existing text rather ` +
        `than overwrite it on a guess`
      )
      if (severity === 'none') severity = 'warn'
    }
  }

  // The server counted a lock the coach claimed and the write did not make. Unlike every
  // heuristic here this is a RECORDED fact, so it is an alert on its own.
  if (lockOverClaims > 0) {
    reasons.push(
      `${lockOverClaims} LOCK OVER-CLAIM(S) recorded — the coach told the student a ` +
      `section was saved and the write did not land`
    )
    severity = 'alert'
  }
