...
// a Mon/Thu cadence written ahead) and have each go live on its own date.
//
// NO daily rebuild is needed — this comment used to say otherwise and cost a lane a
// false blocker (2026-08-11). /blog and /blog/[slug] both set `revalidate = 3600`,
// and generateStaticParams enumerates EVERY slug including future-dated ones, so a
// scheduled post is already a known route that 404s until its date and then
// re-generates to reveal itself within the hour. ISR was chosen over a daily
// rebuild deliberately: no cron, no deploy hook, nothing to forget. In development every
...
export function getPostBySlug(slug) {
  ...
  return {
    ...
    title: data.title ?? slug,
    date: data.date ?? '',
    // Optional `updated: YYYY-MM-DD` frontmatter — a post that gets a substantive
    // revision can declare it, and blogPostingSchema() emits it as `dateModified`
    // (falling back to `date`). Purely additive: posts without it behave as before.
