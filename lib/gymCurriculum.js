...
// Deterministic profile-phrase -> skill mapping (suggestion-engine §1, P2).
export const SKILL_SIGNALS = Object.fromEntries(GYM_SKILLS.map(s => [s.key, s.signals]))
...
// ── Levels ──────────────────────────────────────────────────────────────────
// Milestones, never access locks (design §Levels). Builder/Craftsman = all 8 skills
// of the tier at practiced+; Writer = 4 Tier-3 skills + Portfolio Review (Q4).
export const LEVELS = [
  { key: 'finder',    name: 'Finder',    tier: 0 },
  { key: 'builder',   name: 'Builder',   tier: 1 },
  { key: 'craftsman', name: 'Craftsman', tier: 2 },
  { key: 'writer',    name: 'Writer',    tier: 3 },
]
...
/**
 * Level from the set of practiced+ skill keys. Writer requires 4 Tier-3 skills plus
 * Portfolio Review (Q4). `everStarted` grants Finder after any first session.
 */
