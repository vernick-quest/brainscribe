...
// ── Badge credit — DISPLAY TRUTH ONLY ───────────────────────────────────────
// A `gym_skill_state` row carries BOTH a `state` (practiced | locked_in) and a
// `practiced_source` (session | placement | profile). A row with source 'profile'
// was credited from the student's WRITING PROFILE: the skill showed up in a real
// assignment, so it is genuinely theirs — but they never did a Skill Studio rep for
// it, and it leaves nothing in the portfolio. Collapsing both into "Practiced" made
// a student's own badge wall claim reps they never did (2026-07-25: a student had 2
// skills badged Practiced with zero completed sessions and an empty portfolio).
//
// This is LABELLING ONLY. It does not touch the practiced/locked_in ladder, the
// prereq unlock rule (isUnlocked), or the suggestion engine — profile-credited skills
// still unlock dependants and are still deliberately NOT re-suggested.

/**
 * How a badge was earned, for labelling:
 *   'none'      — no row yet
 *   'writing'   — spotted in their own writing (profile credit, no Studio rep)
 *   'studio'    — a real Skill Studio rep ('session' | 'placement' | anything else)
 *   'locked_in' — the P3 rung. Outranks source: a locked-in badge is evidenced in
 *                 real work however the practiced rung was first earned.
 * Fail-safe direction: a missing/unknown source counts as STUDIO practice, so real
 * practice is never demoted to profile credit.
 */
export function badgeCredit(state, source) {
  if (!state) return 'none'
  if (state === 'locked_in') return 'locked_in'
  return source === 'profile' ? 'writing' : 'studio'
}

/** Student-voice label for a badge credit ("your writing", not "their writing"). */
export function badgeCreditLabel(credit) {
  switch (credit) {
    case 'locked_in': return 'Locked in'
    case 'writing': return 'Spotted in your writing'
    case 'studio': return 'Practiced'
    default: return null
  }
}
...
/**
 * Derive the parent→child FOREST for a tier's skills from `prereqs` — a pure LAYOUT
 * helper for the all-skills browser (no logic change; `isUnlocked`/`missingPrereqs`
 * are untouched). A skill nests under its PRIMARY (first-listed) prerequisite that
 * belongs to the SAME tier group; a skill with no in-tier prerequisite (empty
 * `prereqs`, or every prereq sitting in an earlier tier — e.g. Tier-3 Tone Control,
 * whose only prereq `voice` is Tier 1) is a top-level root. Cross-tier prereqs stay
 * as real unlock gates — they just don't drive nesting inside one tier section.
 * `feeds_into` is the (looser, sometimes asymmetric) inverse hint and is NOT used to
 * nest — `prereqs` is the authoritative unlock edge. Order among siblings and roots
 * follows GYM_SKILLS (week order). Returns [{ skill, children: [ …same shape ] }].
 */
export function getTierSkillTree(tier) {
  const skills = skillsByTier(tier)
  const inTier = new Set(skills.map(s => s.key))
  const nodes = new Map(skills.map(s => [s.key, { skill: s, children: [] }]))
  const roots = []
  for (const s of skills) { // iteration order = week order → preserved among siblings
    const parentKey = (s.prereqs || []).find(pk => inTier.has(pk)) ?? null
    if (parentKey && nodes.has(parentKey)) nodes.get(parentKey).children.push(nodes.get(s.key))
    else roots.push(nodes.get(s.key))
  }
  return roots
}
