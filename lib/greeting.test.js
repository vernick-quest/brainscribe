...
// ── Mode-neutral wording ─────────────────────────────────────────────────────
// People write here for school AND for themselves (sessions.writing_mode, migration 073).
// These greetings are deterministic template literals, not prompt — so this is the one
// place the product's language is fixed in advance, and the one place a regression would
// silently reintroduce "your assignment" for a student writing a story nobody assigned.
describe('greetings are mode-neutral', () => {
  const NAMES = ['owen', 'deon', 'zoe', 'alistair', 'matilda', 'jade']
  // "assignment" and "essay" both presume school. Checked case-insensitively and on the
  // rendered string, so a template literal cannot smuggle one in.
  const SCHOOL_WORDS = /\b(assignment|essay)\b/i

  it('never says "assignment" or "essay" on a fresh session', () => {
    for (const persona of NAMES) {
      const line = newSessionGreeting(persona, 'Sam')
      expect(line, `${persona}: ${line}`).not.toMatch(SCHOOL_WORDS)
    }
  })

  it('never says "assignment" or "essay" when work was already uploaded', () => {
    for (const persona of NAMES) {
      const line = newSessionGreeting(persona, 'Sam', { existingWork: true })
      expect(line, `${persona}: ${line}`).not.toMatch(SCHOOL_WORDS)
    }
  })

  it('still greets by name and stays persona-specific after the rewording', () => {
    // Neutral must not mean generic — the six coaches are the product.
    const lines = NAMES.map(p => newSessionGreeting(p, 'Sam'))
    for (const line of lines) expect(line).toContain('Sam')
    expect(new Set(lines).size).toBe(NAMES.length)
  })
})
