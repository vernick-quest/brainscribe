 ...
 * `resolveComponentTarget` only ever searched the section under the cursor. But component
 * ids are unique across a prose template — `thesis` lives in the introduction, `evidence`
 * in a body paragraph — and the coach is explicitly instructed to revisit the thesis while
 * working on later paragraphs. A [DONE:thesis:new wording] emitted from a body paragraph
 * therefore looked like an UNKNOWN id, and the inexact fallback either discarded the
 * revision or wrote the new thesis into the body's `evidence` slot. Either way the
 * Final Draft kept the old wording, and nothing flagged it: the old thesis's words are
 * still present, so the orphan check's survival ratio stays high.
 *
 * An exact id in ANOTHER section is not ambiguous — it is the coach being precise. Honour
 * it. Only fall back to guessing when the id exists nowhere.
 *
 * @returns {{ paraIdx: number, id: string, exact: boolean, crossSection: boolean } | null}
 */
export function resolveComponentWrite(scaffold, paraIdx, componentId) {
  const sections = scaffold?.components ?? []
  ...
  // Not here — but a real component elsewhere? Search before falling back to a guess.
  const wanted = String(componentId ?? '').trim().toLowerCase()
  for (let i = 0; i < sections.length; i++) {
    ...
    if (hit) {
      console.warn(
        ...
        `paragraph ${paraIdx} — writing it where it actually lives (the coach revisiting an ` +
        `earlier component is normal; misfiling it is how a revision never reaches the draft)`
      )
      return { paraIdx: i, id: hit.id, exact: true, crossSection: true }
    }
  }

  // Nowhere at all — keep the existing inexact behaviour, scoped to the working section.
