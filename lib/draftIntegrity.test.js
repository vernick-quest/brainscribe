...
// ── Recorded facts, added 2026-08-17 ──────────────────────────────────────────
// Both were being written and never read — "recorded but never surfaced" is the
// half-finished shape that lets a signal look healthy.
describe('refusedRevisions (scaffoldWrite guard stamps revisionRefused)', () => {
  const withRefusal = kind => section([
    { ...item('hook', 'The dog barked'), revisionRefused: kind },
    item('context', 'at the mail carrier'),
  ])

  it('a CROSS-SECTION refusal is an alert — the edit did not land where the student was told', () => {
    const r = checkDraftIntegrity([{ scribed_text: 'The dog barked at the mail carrier' }], [withRefusal('cross-section')], { status: 'complete' })
    expect(r.severity).toBe('alert')
    expect(r.refusedRevisions).toEqual([{ id: 'hook', kind: 'cross-section' }])
    expect(r.reasons.join(' ')).toMatch(/REFUSED as cross-section/)
  })

  it('an INEXACT refusal is a warning — the guard preserved existing text', () => {
    const r = checkDraftIntegrity([{ scribed_text: 'The dog barked at the mail carrier' }], [withRefusal('inexact')], { status: 'complete' })
    expect(r.severity).toBe('warn')
    expect(r.refusedRevisions[0].kind).toBe('inexact')
  })

  it('no refusal, no finding', () => {
    const r = checkDraftIntegrity([{ scribed_text: 'The dog barked at the mail carrier' }], [section([item('hook', 'The dog barked'), item('context', 'at the mail carrier')])], { status: 'complete' })
    expect(r.refusedRevisions).toEqual([])
  })
})

describe('lockOverClaims (recorded server-side, migration 070)', () => {
  const clean = [section([item('hook', 'The dog barked'), item('context', 'at the mail carrier')])]
  const paras = [{ scribed_text: 'The dog barked at the mail carrier' }]

  it('is an alert on its own — it is a recorded fact, not a heuristic', () => {
    const r = checkDraftIntegrity(paras, clean, { status: 'complete', lockOverClaims: 1 })
    expect(r.severity).toBe('alert')
    expect(r.lockOverClaims).toBe(1)
    expect(r.reasons.join(' ')).toMatch(/LOCK OVER-CLAIM/)
  })

  it('zero over-claims leaves an otherwise healthy session clean', () => {
    const r = checkDraftIntegrity(paras, clean, { status: 'complete', lockOverClaims: 0 })
    expect(r.ok).toBe(true)
  })
})
