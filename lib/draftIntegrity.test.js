...
describe('checkDraftIntegrity — telling a skipped component from a lost one', () => {
  ...
  it('does NOT alert when the coach deliberately skipped a component (Lyndsay 2026-07-27)', () => {
    ...
    expect(r.severity).toBe('warn')            // visible, but not screaming
    expect(r.droppedComponents).toEqual([])
    expect(r.renderedFromParagraphs).toBe(true)
    expect(r.reasons.join(' ')).toMatch(/unused slots/)
  })
  ...
  it('still alerts on the 2026-07-20 shape — empty slots AND a big shortfall', () => {
    // Guards the downgrade above from going too far: Baron's lost session ALSO had a
    // paragraph row, so "renders from paragraphs" must never be enough to silence it.
    const r = checkDraftIntegrity([{ scribed_text: 'It was late afternoon and I was typing then deleting.' }], [section([
      item('hook', 'It was late afternoon and I was typing then deleting.'),
      skip('reflection'), skip('connection'),
    ])], { currentParagraphIndex: 1, status: 'complete', targetWords: 250 })
    expect(r.severity).toBe('alert')
    expect(r.renderedFromParagraphs).toBe(true)
  })
