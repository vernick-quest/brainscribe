...
import { growComponents, verifyGrowth, reconcileComponentsWrite, growthTypeFor, MAX_GROWTH } from '@/lib/scaffoldGrowth'
...
describe('growComponents — APPEND-ONLY is the whole contract', () => {
  ...
  it('a NARRATIVE grows into another narrative section — never essay slots (Sierra)', () => {
    const { components } = growComponents(sierraNarrative, 1)
    expect(components[1].type).toBe('narrative')
    expect(components[1].items.map(i => i.id)).toEqual(['hook', 'context', 'body', 'closing'])
    // The exact regression: essay slots in a story.
  ...
  // DOCUMENTS the known consequence rather than hiding it: inheriting `narrative` gives the
  // appended scene its own `closing`, so the piece now has two endings and hers sits
  // mid-story. Growth cannot reorder (that would desync paragraph_index from the
  // paragraphs rows), so this is a product call — see growthTypeFor's note and the handback.
  it('KNOWN: inheriting narrative appends a second `closing` (the product call)', () => {
    const { components } = growComponents(sierraNarrative, 1)
    expect(components[1].items.map(i => i.id)).toContain('closing')
  })

  // The alternative, exposed and tested so the product call is a one-line default flip.
  it('opts.type custom + "Scene N" gives a continuation section with NO second ending', () => {
    const { components } = growComponents(sierraNarrative, 2, { type: 'custom', labels: ['Scene 2', 'Scene 3'] })
    expect(components[1].type).toBe('custom')
    expect(components[1].items).toHaveLength(1)
    expect(components[1].items[0].label).toBe('Scene 2')
    expect(components[1].items.map(i => i.id)).not.toContain('closing')
    expect(components[2].items[0].label).toBe('Scene 3')
    // ids still globally unique against her hook/context/body/closing
    const ids = components.flatMap(s => s.items.map(i => i.id))
    expect(new Set(ids).size).toBe(ids.length)
  })
...
describe('growthTypeFor — normalise the essay ladder, inherit everything else', () => {
  it('normalises ONLY the essay ladder to body', () => {
    expect(growthTypeFor('introduction')).toBe('body')
    expect(growthTypeFor('body')).toBe('body')
    expect(growthTypeFor('conclusion')).toBe('body')
  })
  it('inherits the families that are all one type by design', () => {
    expect(growthTypeFor('narrative')).toBe('narrative')
