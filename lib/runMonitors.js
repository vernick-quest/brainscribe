...
// Migration 074 is applied BY HAND, so this deploy can land days before the table exists.
...
// matched — the admin route would have 500'd for as long as 074 sat unapplied, and the
...
export async function recordMonitorRun({ monitor, status, detail = {} }) {
  try {
    ...
    if (error) {
      // Table missing = migration 074 is authored but not pasted yet. Expected between
      ...
      console[expected ? 'warn' : 'error'](
        ...
        (expected ? ' — migration 074 not applied yet' : '')
      )
