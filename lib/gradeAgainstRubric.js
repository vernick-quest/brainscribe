...
export function validateRubricReview(parsed, essay, rubricText) {
  ...
  for (const c of rawCriteria) {
    ...
    // F1 — the criterion label is copied verbatim from the rubric and rendered to
    // the student. Do NOT advice/grade-filter it: a criterion is definitionally
    // "the rubric's own words", and a teacher's imperatively-phrased label ("Provide
    // Context", "Support for Claims") or a per-criterion point value ("Thesis (10
    // points)") quoted verbatim is faithful reporting, not the grader authoring
    // advice. Drop the row only for the two things a criterion can never legitimately
    // be: (1) smuggled model-sentence prose (length cap — the real ghostwrite vector),
    // or (2) an AGGREGATE grade masquerading as a criterion ("TOTAL SCORE: 34/40",
    // "Overall Grade: A") — named-as-aggregate AND grade-shaped.
    if (criterion.length > CRITERION_MAX) continue
    if (/\b(total|overall|final|aggregate|combined|sum)\b/i.test(criterion) && hasGradeShape(criterion)) continue

    let leveled = c.leveled === true
    let matched = cleanLevelName(normLevel(c.matched_level))
    let nextUp = cleanLevelName(normLevel(c.next_level_up))
    ...
    if (leveled) {
      ...
      if (!matchedOk || !nextOk) {
        ...
        // F3 — isVerbatim('') whitelists an empty descriptor, so a NAMED level
        // with no descriptor is an invented level (one the teacher never printed).
        // Blank any such level; if nothing verifiable remains, it isn't leveled.
        if (!matched.descriptor_quote) matched = { name: '', descriptor_quote: '' }
        if (!nextUp.descriptor_quote) nextUp = { name: '', descriptor_quote: '' }
        if (!matched.descriptor_quote && !nextUp.descriptor_quote) leveled = false
      }
    } else {
    ...
    // F1 — location is model-controlled free text shown to the student; a "¶2"
    // is fine, a smuggled model sentence or grade is not. Blank it on a hit.
    let location = asString(c.location).trim()
    if (looksLikeAdvice(location) || hasGradeShape(location)) location = ''
...
// A genuine rubric criterion label is short; a longer value is smuggled prose.
const CRITERION_MAX = 120
...
// F1 — a level `name` ("Exceeds", "Level 4") is copied from the rubric and shown
// to the student, but the safety checks never ran on it. Blank the name if it
// reads as advice or a grade/score; the descriptor still stands on its own.
function cleanLevelName(lvl) {
  if (looksLikeAdvice(lvl.name) || hasGradeShape(lvl.name)) return { name: '', descriptor_quote: lvl.descriptor_quote }
  return lvl
}
...
<assignment_context>
${neutralizeDelimiters(asString(assignmentText).trim()) || '(assignment prompt not provided)'}
</assignment_context>

<rubric_document>
${neutralizeDelimiters(asString(rubricText).trim())}
</rubric_document>

<student_essay>
${neutralizeDelimiters(asString(essay).trim())}
</student_essay>`
}

// F6 — the untrusted rubric/essay are interpolated between literal container
// tags. A forged `</rubric_document>` (or a fake opening tag) inside a document
// would appear to break out of its untrusted container and could be read as
// framing rather than content. Strip the angle brackets off any container tag so
// the text stays inert content, while leaving all other markup the student wrote
// intact.
function neutralizeDelimiters(s) {
  return asString(s).replace(/<\/?(?:rubric_document|student_essay|assignment_context)\s*>/gi, m => m.replace(/[<>]/g, ''))
}
