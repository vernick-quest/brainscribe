...
export function validateRubricReview(parsed, essay, rubricText) {
  ...
  for (const c of rawCriteria) {
    ...
    // F1 — the criterion label is copied verbatim from the attacker-controlled
    // rubric and rendered to the student, but the safety checks never ran on it.
    // A criterion that is itself advice, a grade/score, or smuggled prose is not a
    // real criterion → drop the whole row. (A genuine rubric row label is short; a
    // value past the cap is smuggled prose.)
    if (looksLikeAdvice(criterion) || hasGradeShape(criterion) || criterion.length > CRITERION_MAX) continue
