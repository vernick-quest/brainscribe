...
/** Ensure the student has a gym_progress row (Finder on first touch). Idempotent. */
 ...
 */
export async function awardPracticed(studentId, skillKey, { source = 'session', evidenceRef = null } = {}) {
  const supabase = createServiceClient()
  const { data: existing } = await supabase
