...
import { breachKey, breachProgress, summaryContradictsBreaches } from './auditBreach'
...
describe('summaryContradictsBreaches', () => {
  const breaches = [{ type: 'compose_as_transcription', message_index: 38 }]

  it('flags "no integrity breaches were found" beside real breaches', () => {
    const s = 'Owen coached cleanly throughout the session. There are a few process-level concerns worth noting — but no integrity breaches were found.'
    expect(summaryContradictsBreaches(s, breaches)).toBe(true)
  })

  it('flags "coached cleanly" beside real breaches', () => {
    expect(summaryContradictsBreaches('The coach coached cleanly at every stage.', breaches)).toBe(true)
  })

  it('does NOT flag a clean claim when there are genuinely no breaches', () => {
    expect(summaryContradictsBreaches('Owen coached cleanly; no integrity breaches were found.', [])).toBe(false)
  })

  it('does NOT flag an honest summary that names the breach', () => {
    const s = 'The coach composed a full paragraph and locked it on a rubber-stamp — a compose-as-transcription breach.'
    expect(summaryContradictsBreaches(s, breaches)).toBe(false)
  })

  it('handles missing/garbage input', () => {
    expect(summaryContradictsBreaches(null, breaches)).toBe(false)
    expect(summaryContradictsBreaches('no breaches were found', null)).toBe(false)
  })
})
